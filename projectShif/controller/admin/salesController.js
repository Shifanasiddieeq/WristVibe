...
const loadSales = async (req, res) => {
    ...
    try {
        const orders = await orderSchema.find(filter).populate("products.productId");

        let totalAmount = orders.reduce((total, ord) => total += ord.totalPrice, 0);
...
const downloadSalesreport = async (req, res) => {
    ...
    try {
        ...
        const orders = await orderSchema.find().populate('customer').populate('products.productId');
        ...
        if (format === 'pdf') {
            ...
            const padText = (text, width) => {
                text = String(text);
                return text.length < width ? text + ' '.repeat(width - text.length) : text.slice(0, width);
            };
            ...
            const renderRow = (columns, isHeader = false) => {
                ...
                columns.forEach((col, index) => {
                    doc.text(padText(col.text, col.width), col.x, col.y, { continued: index < columns.length - 1 });
                });
                doc.text('\n'); 
            };
            ...
            reportData.orders.forEach((order) => {
                renderRow([
                    ...
                    { text: `₹${(order.offerDiscount || 0).toFixed(2)}`, width: columnWidths.offerDiscount, x: 70 + columnWidths.date + columnWidths.orderId + columnWidths.customerName + columnWidths.totalAmount + columnWidths.couponDiscount, y: doc.y },
            ...
            worksheet.addRow(['Date', 'Order ID', 'Customer Name', 'Total Amount', 'Coupon Discount','Offer Discount']);
            reportData.orders.forEach((order) => {
                worksheet.addRow([
            ...
            worksheet.addRow([]);
            worksheet.addRow(['Total', '', '', reportData.totalAmount, reportData.totalDiscount]);
